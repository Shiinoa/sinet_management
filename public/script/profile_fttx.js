$(document).ready(function(){ 
    $('#show_profile').click(function(e){ //รับค่า จาก HTML โดย ข้อมูลที่รับ ค่า ต้อง มีประเภท id ในฟังค์ชั่นนั้น จะทำงานก้ต่อเมื่อ คลิ๊ก
        let res_data = $(this).val(); //รับค่า จาก HTML โดย ข้อมูลที่รับ ค่า ต้อง มีประเภท id ในฟังค์ชั่นนั้น
        console.log("Ajax::"+res_data);
        $.ajax({//ส่งข้อมูล JSON แบบ ajax
            url: '../config/profile_popup_data.php', //ลิ้งที่ไปยัง Database
            type: 'POST',//แบบ POST
            ...
            success: function(data) {
                ...
                const mapObj = //ทำการ ดุึง ข้อมูล ใน data ในที่นี้ ใช้ Array ในการเทียบตำแหน่ง 
                    ...
                    document.getElementById("result_profile_fttx").innerHTML =  const_1.replaceAll(/\b(?:ont_index|onu_sn|up_speed|down_speed|u_id|isp|sp_name|vlan_n|vlan_m|u_name|tel_phone|auth_pass|auth_user)\b/gi, matched => mapObj[matched]);
