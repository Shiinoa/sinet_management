        ...
        while ($row_data = $query->fetch(PDO::FETCH_ASSOC)) {
            $result_olt= $db->prepare("SELECT * FROM fttx_profile WHERE p_id = :u_p_id ");
                ...
                $result_olt->execute();
                    ...
                    $index[9] =  $row_data['vlan1'];
                    $index[10] =  $row_data['vlan2'];
                    $index[11] = $row_data['name']; 
                    $index[12] = $row_data['phone_no']; 
                    $index[13] =  $row_data['nonid']; 
                    $index[14] = $row_data['pass']; 
                    $index[15] = $row_data['p_id']; 
                    $index[16] = $row_olt['p_fmt']; 
                } 
            # code...
        }
