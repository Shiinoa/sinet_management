...
if(isset($_REQUEST['insert'])){
        ...
        if(empty($u_id)){
            ...
            try {
                if(!isset($errMsg)){
                    $insertData = $db->prepare("INSERT INTO installed(u_id,isp,user_status,mac_pon,p_id,pmg_model,pmg_mode,name,phone_no,sp,sp_id,node,vlan,vlan_m,type,pon1,pon2,pon3,ont,nonid,pass,pass_onu,dataport,tc_code)
                    VALUES(:u_id,:isp,:user_status,:mac_pon,:p_id_data,:pmg_model,:pmg_mode,:name,:phone_no,:sp,:sp_id,:node,:vlan,:vlan_m,:type,:pon1,:pon2,:pon3,:ont,:nonid,:pass,:pass_onu,:dataport,:tc_code)");
